...
namespace CameraPluginNamespace {
	...
	UInt32 CalculatePosition(StaticFunctionTag* base) {
		...
		_MESSAGE("cam->pos[%f,%f,%f]", cam->pos.x, cam->pos.y, cam->pos.z);
		NiNode* cameraNode = cam->cameraNode;
		_MESSAGE("cameraNode->m_worldTransform[%f,%f,%f]", cameraNode->m_worldTransform.pos.x, cameraNode->m_worldTransform.pos.y, cameraNode->m_worldTransform.pos.z);
